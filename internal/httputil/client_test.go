...
type zeroBackoffRetryer struct{ DefaultRetryer }

func (zeroBackoffRetryer) BackoffDelay(int) time.Duration { return 0 }

func testClient(t *testing.T, srv *httptest.Server) *http.Client {
	t.Helper()
	return NewHTTPClient(srv.Client().Transport, zeroBackoffRetryer{})
}
...
func TestDo_RetryWithBody(t *testing.T) {
	...
	c := testClient(t, srv)
	req, err := http.NewRequestWithContext(t.Context(), http.MethodPut, srv.URL, strings.NewReader("hello"))
	if err != nil {
		t.Fatal(err)
	}
